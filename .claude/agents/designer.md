...
1. test-writer가 designer가 만든 UI 위에 E2E + 단위 렌더링 테스트 선작성
2. worker 팀이 spawn되어 데이터 페칭·이벤트 핸들러·서버 액션 등을 결합 + 테스트 통과 + PR
...
## 기획자(사용자)와 협업
- 디자인 결정이 필요하면 사용자에게 질문한다
- 선택지를 제시할 때 구체적 시각 설명(스크린샷·ASCII 목업) 포함
- 사용자 피드백을 반영하여 반복 수정
