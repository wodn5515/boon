-- 0007_entries_rls.sql
-- 결정 로그 006 §A·§J — entries RLS 정책 정의.
--
-- 사용자별 격리:
--   - entries: 본인 user_id 행만 (auth.uid()).
--
-- 정정-1 (004 §"sfx 라운드 1 🔴 #1") 패턴 그대로:
--   - application-layer `eq(entries.user_id, currentUser.id)` 가 본 방어선.
