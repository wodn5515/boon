...
### §1 후속 — 위젯 B 생일 배지 aria-hidden (2026-05-13)

test-writer가 선축 selector 갱신 후 시나리오 4의 추가 회귀 발견: `getByLabel("생일 오늘")` 배지가 위젯 B 친구 카드 + 위젯 C 친구 카드 양쪽에 동일 노출 → strict 2 elements.

**결정**: 위젯 B의 D-N 배지를 `aria-hidden="true"`로 처리. 시각은 그대로(친구 그리드의 보조 정보), 접근성 트리에서는 제외. "오늘"·"D-N" 강조의 본 의미는 위젯 C가 담당("다가오는 생일" 카드).

**근거**:
- 위젯 B 본 목적 = "친구 그리드 6명 + 받은 신세 수". 생일 D-N은 보조 정보
- 위젯 C 본 목적 = "다가오는 생일 강조". D-N 배지가 핵심 의미
- 같은 정보 양쪽 접근성 트리 노출은 스크린리더 사용자에게 redundant

**거절 대안**:
- (a) spec assertion scope 한정 — spec 매처 복잡도↑
- (b) `.first()` cast — TDD 약화
- (d) 위젯 B 배지 자체 제거 — 시각 정보 손실 (사용자가 친구 그리드에서 생일 임박 친구 한눈에 식별 못함)